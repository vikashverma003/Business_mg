        ...
        <div class="content-wrapper" style="min-height: 1545px;">
          <div class="card">
            <div class="card-body">
                  ...
                  @endif
               <!-- <a href="{{url('admin/addReports')}}" class="btn addLangBtn">
                  Add Reports 
                          </a>-->
                              ...
                              <ul class="navbar-nav">
                                <li class="nav-item dropdown d-none d-lg-flex">
                                  ...
                                  <div class="dropdown-menu navbar-dropdown" aria-labelledby="actionDropdown">
                                    <a href="{{route('EditReports',$user->id)}}" class="dropdown-item">Edit</a>
                                    <a href="{{route('sendNotices',$user->id)}}" class="dropdown-item">Formal Notice</a>
   ...
   function delete_confirmation(id)
  {
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        $.ajax({
          type: 'GET',
          url: "{{route('delete_manager')}}?user_id="+id,
  ...
  function block_confirmation(id, status)
  {
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        $.ajax({
          type: 'GET',
           url: "{{route('block_manager')}}?user_id="+id+"&status="+status,
          success:function(data){
            if(data.success == true)
            {
              swal("Done!", data.message, "success");
